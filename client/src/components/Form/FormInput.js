...
const FormInput = (props) => {
    ...
    const changeInput = async (event)=>{
        ...
        const errors = props.validate==="true"?await Validate_Form(name , value):[];
        setErrors(errors);
    ...
    return(
        <div>
            ...
            <input onChange={changeInput} required type={props.type} name={props.name}
                   className={`bg-${InputColor}-50 border-${InputColor}-500 text-${InputColor}-800 mt-1 px-3 py-2 border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1`}
